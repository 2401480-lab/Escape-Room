...
Assert-True ($ui -match 'enum\s+JournalTab' -and $ui -match 'activeTab' -and $ui -match 'ApplyActiveTab\s*\(') 'ClueJournalUI must remember and reapply the active tab after clue updates.'
Assert-True ($ui -match 'RefreshUI\s*\(\s*\)[\s\S]*EnsureUI\s*\(' -and $ui -match 'SetOpen\s*\(\s*bool\s+isOpen\s*\)[\s\S]*RefreshUI\s*\(') 'ClueJournalUI must rebuild content when clue data changes or the journal opens.'
Assert-True ($ui -match 'OnClueAdded\s*\+=' -and $ui -match 'OnClueAdded\s*-=' -and $ui -match 'HandleClueAdded\s*\(') 'ClueJournalUI must listen for newly collected clues.'
Assert-True ($ui -match 'lastAddedClue' -and $ui -match 'GetDiscoveredCardColor\s*\(') 'ClueJournalUI must remember and highlight the newest collected clue card.'
Assert-True ($ui -match 'CreateClueCard\s*\(\s*clueData\s*,\s*true\s*,\s*clueData\s*==\s*lastAddedClue\s*\)') 'Collected evidence cards must pass newest-clue highlight state into card rendering.'
...
Assert-True ($ui -match 'RectMask2D' -and $ui -notmatch 'Mask\s+mask') 'ClueJournalUI scroll viewports must use RectMask2D so transparent masks cannot hide cards.'
