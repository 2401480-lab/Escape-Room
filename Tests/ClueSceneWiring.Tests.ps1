...
Assert-True ($sceneSetup -notmatch '\[InitializeOnLoad\]') 'Scene setup tool must not auto-run after Unity recompiles scripts because that can reset manually edited clue boxes.'
...
Assert-True ($sceneSetup -notmatch 'EditorApplication\.delayCall' -and $sceneSetup -notmatch 'RepairOpenShowCluesIfNeeded') 'Scene setup tool must not auto-repair an open Show scene because that can reset manually edited clue positions.'
Assert-True ($sceneSetup -notmatch 'ExpectedStoryClueCount\s*=\s*15' -and $sceneSetup -notmatch 'realClueCount\s*<\s*ExpectedStoryClueCount') 'Scene setup must not use clue-count based auto-repair.'
...
Assert-True ($sceneSetup -notmatch 'GenerateStoryClueAssets\s*\(') 'Scene setup must not regenerate ClueData assets automatically because that can overwrite edited clue text.'
...
Assert-True ($sceneSetup -match 'isNewClueObject') 'Scene setup must distinguish newly created clue boxes from existing manually placed clue boxes.'
Assert-True ($sceneSetup -match 'if\s*\(\s*isNewClueObject\s*\)\s*\{[\s\S]*?clueObject\.transform\.position\s*=\s*CluePlacementLayout\.TryGetPosition') 'Scene setup must only assign layout positions when creating new clue boxes.'
