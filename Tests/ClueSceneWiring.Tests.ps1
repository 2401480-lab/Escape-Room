...
Assert-True ($sceneSetup -match 'MenuItem\("Tools/Clues/Setup All Stage Clues"\)' -and $sceneSetup -match 'SetupAllStageClues') 'Scene setup tool must expose a batch setup menu for all stage scenes.'
Assert-True ($sceneSetup -match 'EditorSceneManager\.OpenScene' -and $sceneSetup -match 'EditorSceneManager\.SaveScene') 'Batch setup must open and save all stage scenes.'
