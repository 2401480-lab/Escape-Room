...
Assert-True ($gameOver -match 'GameOverReason\.WrongAnswer' -and $gameOver -match 'GameOverReason\.DeductionTimerExpired' -and $gameOver -match 'GameOverReason\.ChaseTimerExpired') 'Wrong answer, deduction timer expiry, and spacebar QTE timeout must trigger the jumpscare.'
Assert-True ($gameOver -match 'ShouldPlayJumpscare[\s\S]*?return[^;]*GameOverReason\.ChaseTimerExpired[^;]*;') 'ShouldPlayJumpscare must return true for the spacebar QTE timeout reason.'
