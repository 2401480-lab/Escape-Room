...
Assert-True ($bootstrapper -match 'IsRoom02Scene' -and $bootstrapper -match 'scene\.name\s*==\s*"Show"' -and $bootstrapper -match 'scene\.name\s*==\s*"Scene_OperatingRoom"') 'HUD bootstrapper must create Room02 HUD in both Show and direct Scene_OperatingRoom play mode.'
