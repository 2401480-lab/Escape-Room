...
Assert-True ($bootstrapper -match 'EnsureRuntimeObject<ClueJournalUI>' -and $bootstrapper -match 'EnsureRuntimeObject<TimerUI>' -and $bootstrapper -match 'EnsureRuntimeObject<SettingsUI>') 'HUD bootstrapper must create missing HUD runtime UI objects in the Room02 Show scene.'
