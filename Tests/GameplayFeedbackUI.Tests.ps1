...
Assert-True ($popup -match 'StopCoroutine\s*\(\s*fadeRoutine\s*\)' -and $popup -match 'StartCoroutine\s*\(\s*ShowThenFade\s*\(') 'CluePickupPopupUI must cancel an existing popup coroutine before starting a new one.'
Assert-True ($popup -match 'SetPopupActive\s*\(\s*true\s*\)' -and $popup -match 'SetPopupActive\s*\(\s*false\s*\)' -and $popup -match 'SetActive\s*\(\s*active\s*\)') 'CluePickupPopupUI must activate the popup when shown and deactivate it after fade-out.'
