...
Assert-True ($popup -notmatch 'displayDuration|fadeDuration|WaitForSeconds|ShowThenFade|StartCoroutine') 'CluePickupPopupUI must not auto-hide clue popups on a timer.'
Assert-True ($popup -match 'CanvasGroup' -and $popup -match 'alpha') 'CluePickupPopupUI must still control popup visibility through CanvasGroup alpha.'
Assert-True ($popup -match 'Button\s+popupDismissButton' -and $popup -match 'onClick\.AddListener\s*\(\s*DismissPopup\s*\)') 'CluePickupPopupUI must close through an actual clickable UI button.'
Assert-True ($popup -match 'raycastTarget\s*=\s*true' -and $popup -match 'blocksRaycasts\s*=\s*true' -and $popup -match 'interactable\s*=\s*true') 'CluePickupPopupUI must receive popup clicks through the UI event system.'
Assert-True ($popup -match 'IsPopupVisible' -and $popup -match 'Input\.GetMouseButtonDown\s*\(\s*0\s*\)') 'CluePickupPopupUI must expose popup visibility and keep a mouse-click fallback.'
Assert-True ($popup -match 'DismissPopup\s*\(' -and $popup -match 'SetPopupActive\s*\(\s*false\s*\)') 'CluePickupPopupUI must hide the popup when clicked.'
Assert-True ($popup -match 'SetPopupActive\s*\(\s*true\s*\)' -and $popup -match 'SetPopupActive\s*\(\s*false\s*\)' -and $popup -match 'SetActive\s*\(\s*active\s*\)') 'CluePickupPopupUI must activate the popup when shown and deactivate it after a click.'
