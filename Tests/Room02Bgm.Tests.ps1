...
$clipPath = Join-Path $root "Assets/Room02_Operating/Audio/Resources/Room02_Audio/grimyth - You shouldn't have come here.wav"
$clipMetaPath = Join-Path $root "Assets/Room02_Operating/Audio/Resources/Room02_Audio/grimyth - You shouldn't have come here.wav.meta"
...
Assert-True ($script -match "resourcesClipPath\s*=\s*""Room02_Audio/grimyth - You shouldn't have come here""") 'Room02 BGM player must know the Resources fallback clip path.'
Assert-True ($script -match "editorClipAssetPath\s*=\s*""Assets/Room02_Operating/Audio/Resources/Room02_Audio/grimyth - You shouldn't have come here\.wav""") 'Room02 BGM player must know the editor asset fallback path.'
Assert-True ($script -match 'ResolveBgmClip\s*\(' -and $script -match 'LoadConfiguredClip\s*\(' -and $script -match 'Resources\.Load<AudioClip>' -and $script -match 'AssetDatabase\.LoadAssetAtPath<AudioClip>') 'Room02 BGM player must resolve its configured clip at Play time even if the scene reference is stale or missing.'
Assert-True ($script -match 'configuredClip\s*!=\s*null[\s\S]*bgmClip\s*=\s*configuredClip' -and $script -match 'audioSource\.clip\s*=\s*bgmClip') 'Room02 BGM player must prefer the configured recent BGM over an old serialized scene clip.'
