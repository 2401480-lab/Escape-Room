...
### [2026-06-15 17:54:09]
- Show 씬의 15개 단서를 플레이어 시작 위치 바로 앞 5x3 격자로 이동
- 단서 복구/재배치 툴도 같은 시작 위치 격자 좌표를 사용하도록 수정해 다시 흩어지지 않게 개선
- Room02 단서 배치 테스트를 시작 위치 배치 기준으로 갱신

### [2026-06-15 18:21:01]
- 단서 10개 이상 수집 시 표시되는 버튼을 화면 오른쪽 위의 "범인 추적" 버튼으로 변경해 더 쉽게 찾을 수 있도록 개선
- 수집 증거 기록을 "구역 노트 - 단서명" 카드 형태로 정리하고 카드별 텍스트 높이를 고정해 글씨 겹침을 방지
- 용의자 수첩을 스크롤 카드 구조로 바꾸고 하시호 고인 카드는 하시호 관련 단서를 수집한 뒤에만 나타나도록 수정
- 설정창 조작법을 키와 기능이 분리된 행 형태로 정리하고 범인 추적, 팝업 닫기 조작을 추가

### [2026-06-15 18:39:39]
- Room02_BGM이 씬에서 빠져 있거나 클립 참조가 비어 있어도 Play 시작 시 런타임 부트스트랩에서 자동 생성되도록 연결
- Room02BgmPlayer가 AudioSource를 강제로 활성화하고 음소거를 해제한 뒤 BGM 클립을 재탐색해 바로 재생하도록 보강
- Show 씬의 BGM 기본 볼륨을 0.65로 올려 Play 버튼을 눌렀을 때 배경음이 들리도록 조정
- BGM 자동 생성, 클립 복구, 재생 볼륨을 검증하는 테스트를 강화

### [2026-06-15 20:12:41]
- 수첩 패널이 열려도 카드와 칩 영역이 비어 보이던 문제를 수정하기 위해 스크롤 콘텐츠 높이와 칩 너비를 직접 계산하도록 개선
- 수사노트가 수집한 단서를 맨 위에 먼저 보여주고 미수집 단서는 아래로 분리되도록 정리
- 수첩을 열면 마우스 커서를 풀고 보이게 하며, 닫으면 이전 커서 상태로 복구하도록 수정
- 클릭이 어려운 상황에서도 1번/2번 키로 수집 증거와 용의자 수첩을 전환하고 ESC 또는 닫기 버튼으로 닫을 수 있게 개선
- 수첩을 ESC로 닫을 때 설정창이 동시에 열리지 않도록 입력 충돌 방지 처리 추가

### [2026-06-15 20:28:15]
- Room02 인트로 본문을 폐요양 병원 사건 개요, 피해자 유안나, 용의자 3명, 20분 탈출 목표가 포함된 긴 도입 문구로 교체
- 긴 인트로 문구가 한 화면에 보이도록 인트로 패널 크기, 본문 높이, 자동 글자 크기 설정을 조정
- 인트로 검증 테스트가 새 도입 문구와 조작 힌트를 확인하도록 갱신

### [2026-06-15 20:51:43]
- 새 효과음 패키지를 Assets 루트가 아닌 Room02 전용 Resources/Audio/SFX 경로로 이동
- IntroScenarioUI가 Room02 Resources의 효과음을 로드해 인트로 텍스트가 열릴 때만 재생하고 닫히면 정지하도록 연결
- Room02 범위 검증과 인트로 전용 효과음 재생 검증 테스트를 추가

### [2026-06-15 21:32:04]
- 수첩 스크롤 영역이 투명 마스크 때문에 카드가 보이지 않을 수 있던 문제를 RectMask2D 적용으로 수정
- 단서 수집 후 수사 노트와 용의자 수첩이 현재 탭을 유지한 채 즉시 다시 그려지도록 갱신 흐름 보강
- 인트로 문구를 한 화면에 모두 표시하지 않고 여러 페이지로 나누고, 좌우 이전/다음 버튼으로 넘길 수 있게 수정
...
### [2026-06-15 22:58:57]
- Room02 런타임 자동 생성 UI와 진행 관리자에서 UnityEvent가 null로 생성되어 Play 시작 시 예외가 발생하던 문제 수정
- 범인 선택, 추격, 엔딩, 잠금 장치, 구역 전환 이벤트를 기본 빈 이벤트로 초기화해 버튼 및 진행 이벤트 연결이 안전하게 동작하도록 보강

### [2026-06-15 23:02:31]
- plus_young 브랜치의 온보딩 화면만 young 브랜치에 선택 반영
- 온보딩 씬, 시작 화면 배경 이미지, 온보딩 전용 배경음, Play 시작 씬 설정을 추가하고 기존 Room02 Show 씬은 그대로 유지
- Room02 후레쉬, HUD, BGM 파일이 병합 대상에 포함되지 않았는지 확인하고 관련 테스트를 통과
...
### [2026-06-15 23:32:03]
- 온보딩을 첫 씬으로 실행할 때 Room02 HUD/인트로/후레쉬 부트스트랩이 온보딩에서 먼저 소비되던 문제 수정
- Room02 런타임 부트스트랩이 Onboarding에서는 대기하고 Show 씬이 로드되는 시점에 인트로, 후레쉬, HUD, BGM을 생성하도록 변경
- 온보딩에서 Show로 넘어가는 흐름과 Room02 후레쉬/HUD 생성 타이밍을 검증하는 테스트 보강

### [2026-06-15 23:55:59]
- Room02 스토리 단서 15개의 조사 문구와 수첩 업데이트 문구를 최신 폐요양 병원 시나리오에 맞게 갱신
- Show 진입 시 단서가 시작 위치에 몰리지 않고 복도, 병실, 보관실, 분장실, 수술실 좌표로 자동 배치되도록 런타임 배치 레이아웃 추가
- 단서 10개 이상 수집 시 표시되는 버튼명을 범인 찾기로 변경하고, 수사 노트의 최신 획득 단서 카드 강조 표시를 추가
- 현재 Show 씬에서 빠져 있던 범인 시작 오브젝트를 Room02 범위의 char_shadow 에셋으로 복구
...
### [2026-06-16 00:41:26]
- 시간 종료 또는 범인 오답 선택 시 Ch45 모델이 카메라 앞에 튀어나오는 게임오버 점프스케어 연출 추가
- 두둥탁 충격음을 런타임 오디오 클립으로 재생하고, 이후 검은 암전과 빨간 GAME OVER 화면이 표시되도록 수정
- Ch45_nonPBR 모델 에셋을 Room02 모델 폴더에 추가하고 게임오버 연출 검증 테스트를 추가

### [2026-06-16 00:59:46]
- Room02 단서 세팅 도구가 Show 씬을 열거나 메뉴를 실행할 때 단서 에셋과 위치를 자동으로 덮어쓸 수 있던 문제 수정
- 기존 단서 오브젝트는 위치를 보존하고, 새로 생성되는 단서에만 기본 배치 좌표를 적용하도록 변경
- 단서 15개를 모두 수집하면 탈출 열쇠를 자동 획득하도록 스토리 진행 로직 보강
- 단서 자동 초기화 방지와 전체 단서 수집 후 열쇠 획득을 검증하는 테스트 추가

### [2026-06-16 01:25:02]
- Room02 단서 15개 목록 기준을 사용자 제공 프롬프트 문구로 고정
- 벽 메모 조사 텍스트를 "하시호를 위해서. 미안해, 안나. — 쓰레기통 메모와 같은 필체."로 수정
- 단서 배치, 씬 구성, 브금, UI 동작은 변경하지 않고 단서 목록 데이터만 검증하는 테스트 추가
...
### [2026-06-16 02:04:15]
- 단서 수집 매니저 생성 순서가 달라도 StoryProgressManager가 단서 수집 이벤트를 다시 연결하고 기존 수집 단서를 따라잡도록 수정
- 단서 10개 이상 수집 시 범인 추리 버튼이 안정적으로 표시되도록 EndingUI 버튼 갱신 흐름 보강
- 진세웅을 범인으로 맞히면 추격 시작 전에 탈출 열쇠를 즉시 획득하도록 정답 처리 로직 추가

### [2026-06-16 03:04:40]
- Unity 에디터에서 Play 모드 진입을 막던 스크립트 컴파일 캐시 중단 상태를 확인하고 Library/Bee 및 Library/ScriptAssemblies 캐시를 재생성
- Unity 6 배치 컴파일과 로컬 dotnet 빌드에서 C# 컴파일 오류가 없음을 재검증

### [2026-06-16 03:18:34]
- Room02 문 상호작용이 일부 에셋 문을 놓치지 않도록 카메라 정면 레이캐스트에 구형 보조 탐색을 추가
- 문 이름이 door가 아니어도 gate, entrance, exit, 한글 문 이름과 문 크기 패널을 인식하도록 문 판정 보강
- 단서 위 관리자용 화살표가 기본 게임 화면에 표시되지 않도록 런타임 기본 비활성 처리 및 비활성 시 라벨 정리 추가

### [2026-06-16 03:31:55]
- 관리자용 Y 키 건너뛰기를 추가하여 플레이 중 언제든 15개 단서를 모두 수집 처리하고 탈출 열쇠를 즉시 획득하도록 구현
- 단서 박스의 관리자 수집 경로와 StoryProgressManager의 관리자 열쇠 지급 경로를 추가하고 검증 테스트를 보강

### [2026-06-16 03:39:42]
- 관리자용 U 키를 추가하여 타이머를 5초로 낮춘 뒤 기존 추리 시간초과 실패 화면으로 이어지도록 구현
- 범인 찾기 화면을 G 키로 바로 열 수 있도록 단축키를 추가하고 설정 조작 안내를 G 기준으로 갱신
- 관리자 단축키 검증 테스트를 추가하여 U, Y, G 단축키와 기존 타이머/범인 선택 흐름을 함께 확인

### [2026-06-16 03:46:46]
- 커서 없이 범인 선택을 완료할 수 있도록 범인 선택 화면에서 숫자키 1, 2, 3, 4로 용의자를 즉시 선택 확정하는 단축키 추가
- 범인 찾기 버튼과 설정 조작 안내 문구를 `범인찾기 (G)`로 변경하고 용의자 선택 버튼에 숫자 번호를 표시
- 숫자키 용의자 선택과 `범인찾기 (G)` 안내 문구를 검증하는 테스트를 보강

### [2026-06-16 03:54:19]
- 범인 오답 또는 추리 타이머 만료 시 Ch45 모델이 Fast Run 애니메이션으로 카메라를 향해 달려드는 게임오버 연출 추가
- 달려드는 연출이 끝난 뒤 검은 화면과 빨간 GAME OVER 문구가 표시되도록 게임오버 시퀀스 보강
- Fast Run FBX 자산과 메타 파일을 Room02 모델 폴더에 추가하고 점프스케어 검증 테스트 갱신
...
### [2026-06-16 04:19:25]
- Galmuri11 및 Galmuri11-Bold 폰트 파일을 프로젝트와 Resources 폴더에 추가하여 런타임 로드 가능하도록 구성
- `FontHelper`가 Galmuri TMP 자산을 우선 사용하고, 없을 경우 Galmuri TTF로 런타임 TMP 폰트를 생성하도록 변경
- 씬 로드 후 기존 TMP UI 텍스트와 주요 런타임 생성 UI가 Galmuri 폰트를 사용하도록 적용 범위 보강
- Room02 폰트 설정 에디터 메뉴를 Galmuri TMP 생성 및 TMP 기본 폰트 갱신용으로 변경하고 검증 테스트 추가

### [2026-06-16 04:25:54]
- 정답 범인을 선택하면 이미 열쇠가 지급된 상태여도 `열쇠를 얻었습니다` 안내가 다시 표시되도록 열쇠 획득 경로 수정
- 관리자 U 실패 카운트다운 만료 시 `GameOverUI`가 없으면 즉시 생성해 타이머 만료 게임오버 화면이 표시되도록 보강
- Room02 런타임 부트스트랩에서 `GameOverUI`를 자동 생성하도록 추가하고 관련 회귀 테스트 갱신

### [2026-06-16 04:36:11]
- 관리자 Y 단서 건너뛰기 경로에서는 열쇠 획득 자막이 표시되지 않도록 열쇠 안내 억제 구간 추가
- 정답 범인을 맞춘 뒤 표시되는 안내 문구를 `탈출 열쇠를 얻었습니다`로 분리 적용
- Y 강제 수집 중 자동 열쇠 획득 자막이 새어 나오지 않도록 관련 회귀 테스트 보강
...
### [2026-06-16 05:01:27]
- 관리자 Y 스킵은 단서 수집과 범인추리 가능 상태까지만 진행하고 스페이스바 탈출 QTE를 바로 시작하지 않도록 수정
- Y 스킵 후 `StoryPhase.SuspectSelection`으로 넘어가 `범인찾기 (G)` 안내가 뜨는 흐름을 회귀 테스트로 고정
- 탈출 QTE는 정답 범인을 맞췄을 때와 출구 문 상호작용에서만 시작되도록 테스트 설명 정리

### [2026-06-16 11:51:55]
- `Evil Witch Laughter Sound Effects Pack`의 웃음 효과음 1개를 Room02 게임오버용 `Resources` 오디오로 추가
- `GameOverUI` 최종 게임오버 창 표시 시 마녀 웃음 효과음이 한 번 재생되도록 연결
- 게임오버 효과음 파일 위치와 재생 경로를 검증하는 회귀 테스트 보강

### [2026-06-16 12:09:22]
- 플레이 중 하단 HUD에 키캡 형태의 조작 안내를 추가하고 `SHIFT`는 `빨리 달리기`로 항상 표시되도록 구현
- 일반 문과 탈출문 근처에서 `E 문열기` 안내가 나타나도록 `DoorInteractor`와 `EscapeExitDoor`를 새 조작 안내 HUD에 연결
- 설정창 조작법에도 `E 문열기`, `Left Shift 빨리 달리기`를 반영하고 관련 회귀 테스트를 추가

### [2026-06-16 12:19:45]
- `범인찾기 (G)` 버튼 배경을 50% 투명 노란색으로 변경하고 글자 대비를 높여 화면에서 더 잘 보이도록 수정
- 타이머 HUD 위치를 설정 버튼 왼쪽으로 이동해 상단 UI가 서로 겹치지 않도록 조정
- 문 근처에서 `E - 문열기` 안내가 유지되도록 문 근접 감지와 안내 표시 시간을 보강

### [2026-06-16 12:24:59]
- `범인찾기 (G)` 버튼을 오른쪽 상단 설정 영역에서 왼쪽 상단 `용의자 (K)` HUD 버튼 옆으로 이동
- 범인찾기 버튼 위치 회귀 테스트를 수첩 버튼 옆 배치 기준으로 갱신

### [2026-06-16 12:32:29]
- 박스 조사 프롬프트가 거리 밖에서도 남아 있던 문제를 현재 타깃 초기화 방식으로 수정
- `E - 문열기` 조작 안내를 `SHIFT` 안내처럼 항상 표시되도록 변경
- 박스 프롬프트 숨김과 고정 조작 안내 동작을 검증하는 회귀 테스트 추가

### [2026-06-16 12:41:16]
- 프로젝트 전체 구조, Room02 핵심 시스템, 조작법, 테스트 구성, 병합 포인트를 정리한 `PROJECT_DOCUMENTATION.md` 문서 추가

### [2026-06-16 12:46:40]
- 기존 좌하단 조작 안내 패널에 `E - 문열기` 행이 누락된 경우 런타임에서 찾아 생성하도록 보강
- 게임 설정 UI의 기존 및 비활성 TMP 텍스트까지 Galmuri 폰트가 적용되도록 전체 텍스트 적용 패스 추가
- 조작 안내 복구와 설정 폰트 적용을 검증하는 회귀 테스트 추가

### [2026-06-16 13:06:27]
- `Scene_OperatingRoom`을 직접 실행해도 Room02 HUD가 생성되도록 부트스트랩 대상 씬을 보강
- 좌하단 조작 안내가 누락되거나 기존 행 텍스트가 어긋난 경우 `E - 문열기`와 `SHIFT 빨리 달리기`를 런타임에서 다시 맞추도록 수정
- 스페이스바 QTE 시간 초과 시 `GameOverUI`의 범인 돌진 연출과 마녀 웃음소리가 함께 재생되도록 실패 경로 연결

### [2026-06-16 15:27:15]
- 남아 있던 Room02 씬, 단서 에셋, 폰트 에셋, 모델 및 사운드 리소스 변경분을 전체 커밋 대상으로 정리
- 사용자가 요청한 GitHub `main` 반영을 위해 미커밋 에셋 변경과 삭제된 빈 Room02 폴더 구조를 함께 추적
