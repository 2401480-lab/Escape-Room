...
### [2026-06-15 22:10:50]
- 첨부된 병원 공포 이미지를 온보딩 전용 배경 리소스로 추가
- 온보딩 메인 메뉴 버튼을 게임 시작/게임 설정으로 정리하고 하단 우측에 배치
- 온보딩 제목과 버튼에 외곽선, 그림자, 자간을 적용해 공포 분위기를 강화

### [2026-06-15 22:17:54]
- 온보딩 화면 전용 반복 배경음 리소스를 _Shared Resources 경로에 추가
- OnboardingUI가 인트로 효과음이 아닌 전용 BGM을 로드해 온보딩 화면에서 2D 반복 재생하도록 연결
- 온보딩 배경음 리소스, AudioSource 반복 재생 설정, 인트로 효과음 미사용 여부를 검증하는 테스트 보강

### [2026-06-15 22:27:34]
- 온보딩 변경만 유지하고 Room02 Show 씬과 단서, 폰트, 재질의 미커밋 로컬 변경을 young 기준 상태로 복구
- 수사기록, 용의자 수첩, 손전등 HUD가 런타임 부트스트랩으로 생성되는 구조를 확인하고 관련 검증 테스트 통과 확인
- Room02 시작 배치와 범인 에셋 배치 검증 실패 원인이 로컬 Show 씬 변경이었음을 확인하고 정리

### [2026-06-15 22:39:19]
- 온보딩 스크립트의 자체 인트로 패널 생성을 제거하고 게임 시작 버튼이 Show 씬을 바로 로드하도록 수정
- Room02 인트로 소유권을 Assets/Room02_Operating/Clues/IntroScenarioUI.cs로 고정하고 인트로를 3장 구성으로 정리
- 수사기록, 용의자 수첩, 손전등이 Room02 런타임 부트스트랩으로 생성되는지 검증하는 테스트를 보강하고 전체 테스트 통과 확인
