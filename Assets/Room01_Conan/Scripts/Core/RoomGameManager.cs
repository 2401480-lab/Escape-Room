...
// 팀원 통합 시: 이 클래스의 OnRoomCleared 이벤트에 씬 전환 코드를 연결하면 됨
public class RoomGameManager : MonoBehaviour
{
    ...
    void StartOpening()
    {
        ...
        // 수사 수첩 자동 오픈 (시나리오: 시작 시 용의자 3명 등록)
    ...
    void CheckPhaseProgression()
    {
        switch (currentPhase)
        {
            case GamePhase.ExamineBody:
                if (HasAllClues(halfTimeClueIDs))
                    EnterMiddlePhase();
                break;
    ...
    void EnterMiddlePhase()
    {
        currentPhase = GamePhase.ExamineStudy;
    }

    void EnterLatePhase()
    {
        currentPhase = GamePhase.LatePhase;
    }
